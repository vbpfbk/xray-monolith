...
private:
	...
	typedef xr_hash_map<u32, CObject*> ObjectsMap;
	ObjectsMap map_NETID;
...
public:
	...
	{
		if (ID >= decltype(ID)(-1)) //if (ID == u16(-1))
			return (0);

		//return (map_NETID[ID]);
		ObjectsMap::const_iterator It = map_NETID.find(ID);
		return (It != map_NETID.end() ? It->second : NULL);
	}
